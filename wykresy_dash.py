...
def make_figure(file_path, lat_point=55.0, lon_point=19.0):
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.05,
                        subplot_titles=("Średna wysokość fali dla badanego obszaru", f"Fala w zadanym punkcie ({lat_point}, {lon_point})",
                                        f"Wiatr w zadanym punkcie ({lat_point}, {lon_point}) ERA5 i GraphCast"))

    # --- 1️⃣ Среднее по области ---
    fig.add_trace(go.Scatter(x=times, y=ds['fala_era5'].mean(dim=["lat", "lon"]),
                mode='lines', name='ERA5 (среднее по области)', line=dict(width=2, color='black')), row=1, col=1)
    fig.add_trace(go.Scatter(x=times, y=ds['fala_graphcast_swan'].mean(dim=["lat", "lon"]),
                mode='lines', name='GraphCast SWAN (среднее по области)', line=dict(width=2, color='red')), row=1, col=1)
    fig.add_trace(go.Scatter(x=times, y=ds['fala_era5_swan'].mean(dim=["lat", "lon"]),
                mode='lines', name='ERA5 SWAN (среднее по области)', line=dict(width=2, color='green')), row=1, col=1)  # цвет линии)

    # --- 2️⃣ Конкретная точка ---
    fig.add_trace(go.Scatter(x=times, y=ds_point['fala_era5'],
                mode='lines', name=f'ERA5 (точка)', line=dict(dash='dash', width=2, color='black')), row=2, col=1)
    fig.add_trace(go.Scatter(x=times, y=ds_point['fala_graphcast_swan'],
                mode='lines', name=f'GraphCast SWAN (точка)', line=dict(dash='dash', width=2, color='red')), row=2,
                  col=1)
    fig.add_trace(go.Scatter(x=times, y=ds_point['fala_era5_swan'],
                mode='lines', name=f'ERA5 SWAN (точка)', line=dict(dash='dash', width=2, color='blue')), row=2, col=1)

    # --- 3 ветер ---
    fig.add_trace(go.Scatter(x=times, y=ds_point['WS_ERA5_kn'],
                mode='lines', name=f'WS_ERA5_kn', line=dict(dash='dashdot', width=2, color='black')), row=3, col=1)
    fig.add_trace(go.Scatter(x=times, y=ds_point['WS_Graphcast_kn'],
                mode='lines', name=f'WS_Graphcast_kn', line=dict(dash='dashdot', width=2, color='red')), row=3, col=1)
    ...
    fig.update_layout(
        ...
        yaxis1=dict(range=[0, 3], dtick=0.2, showline=True, linewidth=2, linecolor='black', mirror=True, gridcolor='rgba(0,0,0,0.2)'),
        yaxis2=dict(range=[0, 3], dtick=0.2, showline=True, linewidth=2, linecolor='black', mirror=True, gridcolor='rgba(0,0,0,0.2)'),
...
app = Dash(__name__)
app.layout = html.Div([
    html.H3("Просмотр прогнозов GraphCast SWAN", style={'textAlign': 'center'}),
    ...
    html.Div([
        html.Button("◀ Назад", id="prev-btn", n_clicks=0),
        html.Button("Вперёд ▶", id="next-btn", n_clicks=0),
    ], style={'textAlign': 'center', 'margin': '10px'}),

    dcc.Graph(id='plot'),
    html.Div(id='filename', style={'textAlign': 'center', 'fontSize': 18})
])
...
def update_plot(prev, next, lat_value, lon_value, current_name):
    idx = 0
